"""Read a video's technical details with ffprobe and decide how it can be played."""
import json
import subprocess
from dataclasses import dataclass
from pathlib import Path

PROBE_TIMEOUT = 120  # seconds; files live on a NAS, so be generous
# Bump when what's probed or how it's classified changes: rows made by an older
# version are re-probed on the next scan, even if the file itself hasn't changed.
PROBE_VERSION = 1


@dataclass
class ProbeResult:
    container: str | None = None
    video_codec: str | None = None
    audio_codec: str | None = None
    pix_fmt: str | None = None
    width: int | None = None
    height: int | None = None
    duration: float | None = None
    interlaced: bool = False
    error: str | None = None


class ProbeError(Exception):
    pass


def probe(path: Path) -> ProbeResult:
    """Run ffprobe on a file. Raises ProbeError if it can't be read."""
    cmd = [
        "ffprobe", "-v", "error", "-print_format", "json",
        "-show_format", "-show_streams", str(path),
    ]
    try:
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=PROBE_TIMEOUT)
    except subprocess.TimeoutExpired:
        raise ProbeError(f"ffprobe timed out after {PROBE_TIMEOUT}s")
    if proc.returncode != 0:
        raise ProbeError(proc.stderr.strip().splitlines()[-1] if proc.stderr.strip() else "ffprobe failed")
    return parse_probe(json.loads(proc.stdout))


def parse_probe(data: dict) -> ProbeResult:
    streams = data.get("streams", [])
    fmt = data.get("format", {})
    # Skip embedded cover art, which ffprobe reports as a video stream.
    video = next(
        (s for s in streams
         if s.get("codec_type") == "video" and not s.get("disposition", {}).get("attached_pic")),
        None,
    )
    audio = next((s for s in streams if s.get("codec_type") == "audio"), None)

    duration = fmt.get("duration") or (video or {}).get("duration")
    return ProbeResult(
        container=fmt.get("format_name"),
        video_codec=video.get("codec_name") if video else None,
        audio_codec=audio.get("codec_name") if audio else None,
        pix_fmt=video.get("pix_fmt") if video else None,
        width=video.get("width") if video else None,
        height=video.get("height") if video else None,
        duration=float(duration) if duration else None,
        interlaced=bool(video) and video.get("field_order") in ("tt", "bb", "tb", "bt"),
    )


# What browsers can decode. HEVC is left out on purpose: support depends on the
# viewer's hardware, so it's safer to transcode it.
BROWSER_VIDEO = {"h264", "vp8", "vp9", "av1"}
BROWSER_AUDIO = {"aac", "mp3", "opus", "vorbis", "flac"}
# Codecs ffmpeg can copy into an MP4 container without re-encoding.
MP4_VIDEO = {"h264", "vp9", "av1"}
MP4_AUDIO = {"aac", "mp3", "opus", "flac"}
# Browsers only handle 8-bit 4:2:0 H.264 (10-bit "Hi10P" won't play).
H264_PIX_FMTS = {"yuv420p", "yuvj420p"}


def _is_mp4_family(container: str | None) -> bool:
    names = set((container or "").split(","))
    return bool(names & {"mp4", "mov"})


def classify(result: ProbeResult, extension: str) -> str:
    """Pick the cheapest way to get this file into a browser.

    direct      send the file as-is
    remux       copy the streams into an MP4 container (fast, no quality loss)
    transcode   re-encode with ffmpeg
    unsupported no playable video
    """
    if result.error or not result.video_codec:
        return "unsupported"

    vcodec, acodec = result.video_codec, result.audio_codec
    video_ok = vcodec in BROWSER_VIDEO and (vcodec != "h264" or result.pix_fmt in H264_PIX_FMTS)
    audio_ok = acodec is None or acodec in BROWSER_AUDIO

    # Interlaced video shows combing artifacts unless ffmpeg deinterlaces it.
    if result.interlaced or not (video_ok and audio_ok):
        return "transcode"

    ext = extension.lower().lstrip(".")
    if _is_mp4_family(result.container) and ext in ("mp4", "m4v", "mov"):
        return "direct"
    if ext == "webm" and vcodec in ("vp8", "vp9", "av1"):
        return "direct"
    if vcodec in MP4_VIDEO and (acodec is None or acodec in MP4_AUDIO):
        return "remux"
    return "transcode"

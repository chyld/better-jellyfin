...
def test_scan_finds_every_video(conn, library, fake_probe):
    ...
    assert result == {"total": 13, "added": 13, "updated": 0, "removed": 0, "unchanged": 0, "failed": 0}
...
def test_rescan_without_changes_probes_nothing(conn, library, fake_probe):
    ...
    assert result == {"total": 13, "added": 0, "updated": 0, "removed": 0, "unchanged": 13, "failed": 0}
...
def test_rescan_picks_up_added_changed_and_deleted_files(conn, library, fake_probe, media_root):
    ...
    assert result == {"total": 12, "added": 1, "updated": 1, "removed": 2, "unchanged": 10, "failed": 0}
    found = items(conn, library)
    assert "Personal/Camcorder/clip02.avi" not in found
